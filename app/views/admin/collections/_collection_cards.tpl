...
Are you sure about that?{/t}{/capture}
					<li>{a_remote action="collection_cards/remove" collection_id=$collection id=$ccard _method=post _confirm=$confirm _class="btn btn-sm btn-danger btn-xs"}{!"minus-circle"|icon}</i>{/a_remote}</li>
				</ul>
			</li>
	{/foreach}
</ul>
{a action="collection_cards/create_new" collection_id=$collection _class="btn btn-default"}{!"plus-circle"|icon}</i> {t}Add a product into the collection{/t}{/a}
