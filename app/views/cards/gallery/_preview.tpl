...
<figure class="gallery__preview js_gallery_trigger">
	<a href="{$image|img_url:$geometry_detail}" title="{if $image->getDescription()}{$image->getDescription()}{/if}" data-size="{$image|img_width:$geometry_detail}x{$image|img_height:$geometry_detail}" data-preview_for="{$image->getId()}">
		<img {!$image|img_attrs:$geometry_preview} alt="{$image->getName()}" class="img-fluid">
	</a>
</figure>
{/content}
