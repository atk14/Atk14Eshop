					...
					{else}
						...
						<div class="add-to-cart-widget">
							...
							{if $price && $price->priceExists() && !WatchedProduct::IsWatchedProduct($product,$logged_user)}
								<p>
									{a action="watched_products/create_new" product_id=$product _class="btn btn-outline-primary" _rel="nofollow"}{!"dog"|icon} <span class="link__text">{t}Informovat o naskladnění{/t}{/a}</span>
								</p>
							{else}
