	...
	{foreach $tree as $node}
		...
		{if $node->hasChilds()}
			<li class="nav-item nav-item--has-submenu">
				...
				<span class="expander {if $active} {else} collapsed{/if}" role="button" id="sidebar_menu_item_{$node->getId()}" data-toggle="collapse" data-target="#sidebar_submenu_{$node->getId()}" aria-expanded="{if $active}true{else}false{/if}" aria-controls="sidebar_submenu_{$node->getId()}" aria-label="{t}Expand submenu{/t}">{!"chevron-down"|icon}</span>
